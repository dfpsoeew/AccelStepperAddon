...
#define SETMINPULSEWIDTH_STEPPER 0x12
#define SETENABLEPIN_STEPPER 0x13
#define SETPINSINVERTED_STEPPER 0x14
...
const char MSG_SETMINPULSEWIDTH_STEPPER[] PROGMEM = "STEPPER[%d]->setMinPulseWidth(%d);\n";
const char MSG_SETENABLEPIN_STEPPER[] PROGMEM = "STEPPER[%d]->setEnablePin(%d);\n";
const char MSG_SETPINSINVERTED1_STEPPER[] PROGMEM = "STEPPER[%d]->setPinsInverted(%d, %d, %d);\n";
const char MSG_SETPINSINVERTED2_STEPPER[] PROGMEM = "STEPPER[%d]->setPinsInverted(%d, %d, %d, %d, %d);\n";
...
// Saves unsigned integer and byte array in same memory location
typedef union
{
	unsigned int number;
	byte bytes[2];
...
public:
	void commandHandler(byte cmdID, byte* dataIn, unsigned int payloadSize)
	{
		...
		case SETMINPULSEWIDTH_STEPPER:
		{
			...
			value.bytes[0] = dataIn[1];
			value.bytes[1] = dataIn[2];
			// Call setMinPulseWidth() method from Stepper Library
			steppers[stepperID]->setMinPulseWidth(value.number);
			// Print debug string
			debugPrint(MSG_SETMINPULSEWIDTH_STEPPER, stepperID, value.number);
			// Send response
			sendResponseMsg(cmdID, 0, 0);
			break;
		}

		case SETENABLEPIN_STEPPER:
		{
			// Get stepper ID from dataIn
			byte stepperID = dataIn[0];
			byte enablePin = dataIn[1];
			// Call setEnablePin() method from Stepper Library
			steppers[stepperID]->setEnablePin(enablePin);
			// Print debug string and pins
			debugPrint(MSG_SETENABLEPIN_STEPPER, stepperID, enablePin);
			// Send response
			sendResponseMsg(cmdID, 0, 0);
			break;
		}

		case SETPINSINVERTED_STEPPER:
		{
			// Get stepper ID from dataIn
			byte stepperID = dataIn[0];
			if (payloadSize == 4) // direction, step, enable
			{
				bool directionInvert = dataIn[1];
				bool stepInvert = dataIn[2];
				bool enableInvert = dataIn[3];
				// Call setPinsInverted() method from Stepper Library
				steppers[stepperID]->setPinsInverted(directionInvert, stepInvert, enableInvert);
				// Print debug string and pins
				debugPrint(MSG_SETPINSINVERTED1_STEPPER, stepperID, directionInvert, stepInvert, enableInvert);
			}
			else if (payloadSize == 6) // pin1, pin2, pin3, pin4, enable
			{
				bool pin1Invert = dataIn[1];
				bool pin2Invert = dataIn[2];
				bool pin3Invert = dataIn[3];
				bool pin4Invert = dataIn[4];
				bool enableInvert = dataIn[5];
				// Call setPinsInverted() method from Stepper Library
				steppers[stepperID]->setPinsInverted(pin1Invert, pin2Invert, pin3Invert, pin4Invert, enableInvert);
				// Print debug string and pins
				debugPrint(MSG_SETPINSINVERTED2_STEPPER, stepperID, pin1Invert, pin2Invert, pin3Invert, pin4Invert, enableInvert);
			}
			else
			{
				debugPrint("Invalid payload size for SET_PINS_INVERTED");
			}
			// Send response
			sendResponseMsg(cmdID, 0, 0);
			break;
